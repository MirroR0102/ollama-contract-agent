...
from config import LLM_MODEL_NAME, OLLAMA_HOST
from vector_store import get_retriever

# 本地大模型实例：temperature=0 保证审查/问答严谨稳定；
# num_ctx=4096 控制上下文窗口（6GB 显存跑 7B 模型，过大会溢出到 CPU 拖慢推理）；
# num_predict 限制单次最长输出，防止长答拖慢演示。
llm = ChatOllama(
...
def kb_query(user_query: str, top_k: int = 3, emit=None, sources: list = None,
    ...
    """
    retriever = get_retriever(top_k, sources=sources, owner=owner)
